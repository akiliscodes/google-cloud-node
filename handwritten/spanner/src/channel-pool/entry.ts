...
export class ChannelEntry {
  ...
  private activeRwTransactions = 0;
  ...
  activeReadWriteTransactionCount(): number {
    return this.activeRwTransactions;
  }

  /**
   * Alias for activeReadWriteTransactionCount() for backward compatibility.
   */
  activeRwCount(): number {
    return this.activeReadWriteTransactionCount();
  }
   ...
   * Touches the activity timestamp only when dropping to idle (0 in-flight),
   * providing the baseline for idle channel reaping without per-RPC clock calls.
   */
  ...
  /**
   * Increments the active Read/Write transaction counter and touches the activity timestamp.
   */
  incrementActiveRw(): void {
    this.activeRwTransactions += 1;
    this.touchActivity();
  }

  /**
   * Decrements the active Read/Write transaction counter and touches the activity timestamp.
   */
  decrementActiveRw(): void {
    if (this.activeRwTransactions > 0) {
      this.activeRwTransactions -= 1;
      this.touchActivity();
    }
  }
...
/**
 * Token held by an active Read/Write transaction to prevent premature channel closure during draining.
 */
export class RwTransactionAffinityGuard {
  private readonly entry: ChannelEntry;
  private disposed = false;

  constructor(entry: ChannelEntry) {
    this.entry = entry;
    this.entry.incrementActiveRw();
  }
  ...
  dispose(): void {
    ...
    this.disposed = true;
    this.entry.decrementActiveRw();
  }
}

/**
 * Short-lived leased channel reference returned by channel selection.
 *
 * Tracks an active in-flight RPC, provides access to the underlying physical channel
 * and logical slot, and decrements in-flight accounting upon disposal.
 */
export class ChannelLease {
  ...
  constructor(entry: ChannelEntry) {
    this.entry = entry;
    this.entry.incrementInFlight();
  }

  get channel(): grpc.Channel {
    return this.entry.channel;
  }

  get logicalChannelId(): number {
    return this.entry.logicalChannelId;
  }
  ...
  /**
   * Creates an active Read/Write transaction affinity guard on this channel entry.
   */
  rwAffinityGuard(): RwTransactionAffinityGuard {
    return new RwTransactionAffinityGuard(this.entry);
  }

  /**
   * Completes the lease, decrementing the in-flight RPC counter on the channel entry.
   */
  dispose(): void {
    if (this.disposed) {
      return;
