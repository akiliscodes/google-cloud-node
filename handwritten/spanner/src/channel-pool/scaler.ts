...
/**
 * Sweeps draining channels and closes idle channels whose draining requirements are met.
 *
 * Closure Rules:
 * 1. Channels with in-flight RPCs remain in DRAINING state.
...
export async function dialAndPrimeChannel(
  ...
  const channel = channelFactory(target, credentials, channelOptions);
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const serialize = (val: any) => {
        if (protos?.google?.spanner?.v1?.ExecuteSqlRequest?.encode) {
          return Buffer.from(
      ...
      const deserialize = (buf: Buffer) => {
        if (protos?.google?.spanner?.v1?.ResultSet?.decode) {
          return protos.google.spanner.v1.ResultSet.decode(buf);
        }
